...
- **V35.32** : **refonte du menu Export → rapport pro**. Popover réorganisé en 3 blocs : (1) **Infos du rapport** facultatives et mémorisées en `localStorage` (`rep_repSite/Oper/Ref/Note`) ; (2) **Rapport complet (PDF)** via `generateReport()` — injecte une feuille `@media print` (`#repcss`, style clair, `body>*:not(#report)` masqué) et un `#report` rempli par `buildReportHTML(gimg)`, puis `window.print()` → « Enregistrer en PDF » iOS ; (3) CSV/PNG conservés. Le rapport A4 contient : en-tête CETIM + date/réf/site/opérateur, tableau **indices** (LAeq/LCeq/LZeq, LAFmax/min, LCpeak, L10/50/90, LAE-SEL, ΔLF, durée), **conditions d'acquisition** (micro, Fe, FFT, fenêtre, overlap, source 1/3 oct, pondérations fréq./temp., correction d'amplitude `S.offset`, écran exporté), image du graphe courant (`buildGraphCanvas().toDataURL`, préchargée avant `print`), **spectre par tiers d'octave** (colonnes), commentaire, et pied avec mention **non-métrologique**. CSS `.ep-in` pour les champs. Aide FR MAJ.
