...
- **V35.34** : **rapport multi-figures**. Trois cases à cocher (Spectre / Spectrogramme / Évolution, sélection mémorisée `rep_figSpec/Sgram/Hist`) : chaque figure est **rendue à la demande depuis les données courantes même si l'écran n'est pas affiché** (`figureDataURL(kind)` appelle `drawSpectrum`/`drawSgram`/`drawHistory` hors écran — les 3 canvases sont dimensionnés par `sizeCanvas` via repli graphzone et alimentés en continu, puis `toDataURL`). `drawActive(true)` restaure l'écran courant après. `buildReportHTML(figs)` boucle sur les figures (chacune sa section titrée) ; ligne « Écran exporté » retirée. Le spectrogramme est rendu selon le mode courant (`S.sg3d`).
